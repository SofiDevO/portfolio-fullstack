...
const DashboardPanel = () => {
    ...
    const Overlay = ()=>{
        document.querySelector("dialog").addEventListener("click",()=>{
            setOpenOverlay.setAttribute("", "open");
        } )
    }
	return (
		<div className="bg-primary10 h-auto min-h-[100dvh] w-[14rem] py-10 flex flex-col items-center gap-10">
			<div className="px-4 flex flex-col gap-3">
				<img
					...
					alt="Hola Soy una imagen"
					className="w-[150px] h-[150px] min-w-[150px] min-h-[150px] rounded-full "
				/>
				<buuton className="rounded-full text-sm text-center px-3 py-3 bg-primary60 cursor-pointer hover:opacity-[0.75]">
					Actualizar perfil
				</buuton>
			</div>
			...
			<dialog  className="bg-primary10 *:text-white px-16 py-10">
				{/* Componenet Formulario Formulario */}
                <DashboardForm/>
				<form method="dialog">
					<button>OK</button>
				</form>
